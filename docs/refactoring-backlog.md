...
- [ ] **13. Source-space char-index resolver duplication (web).** Surfaced during item #6 simplify pass.
  `useEditorLineMouse.getSourceCharIndex` (overlay+textarea preferred, fall back to directive content + `mapDisplayOffsetToSource`) and `hitTestLineFromPoint` in `web/src/editor/TextMeasure.ts:297` (used by `useEditorInteractions.ts`) are parallel resolvers. Extend `hitTestLineFromPoint` to optionally accept segments and return source-space offsets so both call sites share it.
