...
- [x] **14. `splitLineOnNewline` ↔ `splitLine` scaffolding duplication (both platforms).** _(done; small extraction)_
  Extracted a private `splitLineByContent(lineIndex, line, beforeContent, afterContent, preserveCheckedAfter)` helper that owns the shared mechanics (`splitNoteIds` + `updateContent` first half + `createNewLineWithPrefix`). Both `splitLineOnNewline` and the cursor≥prefix branch of `splitLine` route through it; the cursor-within-prefix branch of `splitLine` stays inline since it's a text-level split with no prefix continuation. Documented in the parity doc.

- [x] **15. `convertSourcePrefix` runs on no-op IME syncs (both platforms).** _(done)_
  Replaced the `if (contentChanged) clearSelection() … if (contentChanged) markContentChanged()` shape with an early-return when `line.content === newContent`. The no-op path now does only `line.updateContent(...)` + `notifyChange()` — skipping prefix conversion, sentinel stamp (Android), selection clear, and `markContentChanged`. Same observable behavior, less per-keystroke work.

- [x] **12. Time-picker dialog + time formatting duplication.** _(done)_
  - Extracted `ui/components/TimePickerDialog.kt` (`TimePickerDialog(initialHour, initialMinute, is24Hour, onConfirm(hour, minute), onDismiss)`); both `DateTimePicker.kt` and `AlarmStageRow.StageTimePicker` now call it.
  - Added `util/DateTimeUtils.kt` top-level `formatTimeOfDay(context, Date)` and `formatTimeOfDay(context, hour, minute)` helpers; `AlarmStageRow.kt`, `DateTimePicker.kt`, and `service/AlarmUtils.kt` now use them.
  - Promoted `MINUTE_MS`/`HOUR_MS` to top-level `const` in `util/DateTimeUtils.kt`. `AlarmStageRow.kt` imports them; `RelativeUnit` (`RecurrenceConfigSection.kt`) and `AlarmUtils.calculateSnoozeEndTime` adopt them too. Other inline `60 * 1000L` sites (`ScheduleAlarmReceiver`, `ScheduleWorker`, `Alarm`, `EditSessionManager`, `CacheLogger`) deferred.
