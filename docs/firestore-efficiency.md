...
The diagnostic that drives this work is `FirestoreUsage` — every read and write is bucketed by operation name and event type (DOC_GET, GET_DOCS, LISTENER_INITIAL_FRESH/CACHED, LISTENER_UPDATE_FRESH/CACHED, LISTENER_LOCAL_ECHO, PULL_DELTA, PULL_FULL_REPAIR, COUNT_AGGREGATION, SET, UPDATE, DELETE, BATCH_COMMIT, TRANSACTION). The "Billed" section of the report shows what the user actually pays for; the "Local-only" section shows cache hits and echoes. If a new operation isn't instrumented, regressions hide.

The notes-collection listener was retired in favor of a signal+pull architecture (see `docs/note-sync-pull-migration.md`). Look for `NoteStore.pull.*` entries in the report rather than `NoteStore.listener` — the former bills exactly the delta size, the latter no longer fires for the notes collection (only for the 1-doc `users/{uid}` signal).
