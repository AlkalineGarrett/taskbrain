...
describe('LastSyncStorage', () => {
  ...
  it('returns Timestamp(0, 0) for unset user', () => {
    const ts = LastSyncStorage.read('NOTES', 'uid-1')
  ...
  it('round-trips a value via ms encoding', () => {
    ...
    LastSyncStorage.write('NOTES', 'uid-1', original)
    const read = LastSyncStorage.read('NOTES', 'uid-1')
  ...
  it('isolates per-user watermarks', () => {
    LastSyncStorage.write('NOTES', 'uid-a', new Timestamp(100, 0))
    LastSyncStorage.write('NOTES', 'uid-b', new Timestamp(200, 0))
    expect(LastSyncStorage.read('NOTES', 'uid-a').seconds).toBe(100)
    expect(LastSyncStorage.read('NOTES', 'uid-b').seconds).toBe(200)
  })

  it('clear() removes the watermark so the next read returns 0', () => {
    LastSyncStorage.write('NOTES', 'uid-1', new Timestamp(500, 0))
    LastSyncStorage.clear('NOTES', 'uid-1')
    const ts = LastSyncStorage.read('NOTES', 'uid-1')
    expect(ts.seconds).toBe(0)
    expect(ts.nanoseconds).toBe(0)
  })
